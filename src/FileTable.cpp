...
tree_node_<FILE_ITEM>* CFileTable::AddItem(const char *path)
{
	...
	item.nPathLen = (unsigned int)strlen(item.path);  //path length
...
int CFileTable::RenameDirectory(const char *pathFrom, const char *pathTo)
{
    ...
    if (dotDirectoryPathFrom == 0) return -1;
    ...
    if (dotDirectoryPathTo == 0) return -1;
    ...
    if (backDirectoryPathFrom == 0) return -1;
    ...
    if (backDirectoryPathTo == 0) return -1;
...
int CFileTable::RemoveFolder(const char *path)
{
    ...
    if (RemoveDirectory(path) != 0) {
        ...
        if (dotDirectoryPath == 0) return -1;
        ...
        if (backDirectoryPath == 0) return -1;
